# login_popup.py
# ============================================================
# LOGIN POPUP MODULE (FULLY FIXED & INDENTED)
# ============================================================
...
class ParticipantLoginPopup:
    ...
    def _on_continue_clicked(self):
        ...
        entries = self.data_manager.get_participant_entries(pid)
        if not entries:
            messagebox.showwarning("No Entries", f"No entries found for {pid}. Creating new entry…")
            entry_folder, sessions = self.data_manager.create_new_entry(pid)
    ...
    # --- ENTRY SELECTION (Properly Indented) ---
    def _show_entry_selection_popup(self, entries):
        self.popup = tk.Toplevel(self.parent)
        self.popup.title("Select Entry")
        self.popup.geometry("600x350") 
        self.popup.resizable(False, False)
        self.popup.grab_set()
        self.popup.configure(bg=COLORS['bg_main'])

        tk.Label(self.popup, text="Select Entry:", font=FONTS['title'],
                 bg=COLORS['bg_main'], fg=COLORS['text_primary']).pack(padx=10, pady=(15, 5))

        content_container = tk.Frame(self.popup, bg=COLORS['bg_main'])
        content_container.pack(padx=20, pady=10, fill=tk.BOTH, expand=True)
        ...
        tk.Button(
            content_container, text="SELECT", font=FONTS['button'],
            bg=COLORS['success'], activebackground=COLORS['success_active'],
            fg=COLORS['text_primary'], width=12, height=4, relief=tk.RAISED, bd=4,
            command=select
        ).pack(side=tk.LEFT, padx=(10, 30), pady=20)

        list_frame = tk.Frame(content_container, bg=COLORS['bg_frame'])
        list_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        scrollbar = tk.Scrollbar(list_frame, width=30)
        scrollbar.pack(side=tk.RIGHT, fill=tk.Y)

        listbox = tk.Listbox(
            list_frame, font=FONTS['label_1'], bg=COLORS['bg_frame'],
            fg=COLORS['text_primary'], yscrollcommand=scrollbar.set,
            activestyle='none', selectbackground=COLORS['success'],
    ...
    # --- SESSION SELECTION (Properly Indented) ---
    def _show_session_selection_popup(self, sessions):
        ...
        tk.Label(self.popup, text="Select Session:", font=FONTS['title'],
                 bg=COLORS['bg_main'], fg=COLORS['text_primary']).pack(padx=10, pady=(15, 5))

        container = tk.Frame(self.popup, bg=COLORS['bg_main'])
        container.pack(padx=20, pady=10, fill=tk.BOTH, expand=True)

        # Safety check for sessions list
        default_val = sessions[0] if sessions else ""
        session_var = tk.StringVar(value=default_val)

        def start():
            self.popup.destroy()
            self.on_success(self.selected_participant, self.selected_entry_index, session_var.get())

        tk.Button(
            container, text="START\nSESSION", font=FONTS['button'],
            bg=COLORS['success'], activebackground=COLORS['success_active'],
            fg=COLORS['text_primary'], width=12, height=4, relief=tk.RAISED, bd=4,
            command=start
        ).pack(side=tk.LEFT, padx=(10, 30), pady=20)

        radio_frame = tk.Frame(container, bg=COLORS['bg_main'])
        radio_frame.pack(side=tk.LEFT, fill=tk.Y, pady=20)

        style = ttk.Style()
        style.configure('Session.TRadiobutton', font=FONTS['label_1'],
                        background=COLORS['bg_main'], foreground=COLORS['text_secondary'])

        for session in sessions:
            ttk.Radiobutton(
                radio_frame, text=session.upper(), variable=session_var,
                value=session, style='Session.TRadiobutton'
